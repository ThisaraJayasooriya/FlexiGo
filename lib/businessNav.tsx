import { NavItem } from "@/app/components/BottomNav";

const RESTRICTED_NAV_IDS = new Set(["jobs", "create", "applications"]);

const baseBusinessNavItems: NavItem[] = [
  {
    id: "home",
    label: "Home",
...
export function getBusinessNavItems(isVerified: boolean): NavItem[] {
  ...
  return baseBusinessNavItems.map((item) =>
    RESTRICTED_NAV_IDS.has(item.id) ? { ...item, disabled: true } : item
  );
}
